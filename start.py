...
def add_user() -> Any:
    if not request.json or "username" not in request.json or "password" not in request.json:
        return jsonify({"ERROR": "Invalid data, please give username and password"}), 400
...
def add_review(username: str) -> Any:
    ...
    if not request.json or "name" not in request.json or "year" not in request.json or "review" not in request.json:
    ...
    if film is None:
        return jsonify({"ERROR": "This film does not exist"})
...
def add_mark(username: str) -> Any:
    ...
    if not request.json or "name" not in request.json or "year" not in request.json or "mark" not in request.json:
    ...
    if film is None:
        return jsonify({"ERROR": "This film does not exist or you mark less 0 or more zero, check it"})
